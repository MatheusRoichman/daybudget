...
export default async function CampaignsPage() {
  ...
  function renderCampaigns(list: CampaignWithExpenses[], status: CampaignStatus) {
    if (list.length === 0) {
      return (
        <p className="text-on-surface-variant text-sm py-12 text-center font-sora">
          Nenhuma campanha {status === "active" ? "ativa" : status === "future" ? "futura" : "encerrada"}.
        </p>
      );
    }
    ...
    return (
      <div className="flex flex-col gap-6">
        {list.map((c) => {
          ...
          const elapsed = status === "active" ? getCurrentDay(c, today) : status === "ended" ? days : 0;
          ...
          return (
            <CampaignListItem
              ...
              totalDays={days}
              currentDay={status === "active" ? getCurrentDay(c, today) : undefined}
              totalSpent={spent}
              daysUntilStart={
                status === "future"
